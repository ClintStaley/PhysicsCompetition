...
public class BounceEvaluator extends Evaluator {
    ...
    * ball, or the ball going out of bounds. in the case of a bounce, the velocityX
    * and velocityY of the ball are the velocities after the bounce, the
    * obstacleIdx describes the obstacle that was hit, or -1 if the event is a
    * starting event or an out of bounds event.
    */
   ...
   private EvlPut evaluate(Submit sbm) throws Exception {
      ...
      LinkedList<Obstacle> obstacles = new LinkedList<Obstacle>(Arrays.asList(prms.obstacles));
      ...
      for (int i = 0; i < numBalls; i++) {
         ...
         BounceEvent startEvent = new BounceEvent(STARTING_HEIGHT, sbmData[i].speed);
