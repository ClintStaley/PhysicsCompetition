...
export class BounceOld3DView extends React.Component {
   ...
   constructor(props) {
      ...
      this.state = BounceOld3DView.setOffset(
         BounceOld3DView.getInitState(props.movie), props.offset);
   }

   // Create standard room with center of far wall at origin
   static buildRoom() {
      let concreteMat = BounceOld3DView.concreteMat;
      let flatSteelMat = BounceOld3DView.flatSteelMat;

      let roomDim = 3 * BounceOld3DView.rigSize + 2;  // big boundaries around rig
   ...
   static getInitState(movie) {
      const rigSize = BounceOld3DView.rigSize;
      const ballRadius = BounceOld3DView.ballRadius
      const ballSteps = 16;
      ...
      let pBase = new THREE.Mesh(new THREE.BoxGeometry(pistonHeight,
          pistonWidth, pistonDepth),BounceOld3DView.flatSteelMat);
      ...
      let pCyl = new THREE.Mesh(new THREE.CylinderGeometry(cylinderWidth,
          cylinderHeight, cylinderLength),BounceOld3DView.flatSteelMat);
      ...
      let pFace = new THREE.Mesh(new THREE.BoxGeometry(pistonHeight,
          faceWidth, pistonDepth),BounceOld3DView.flatSteelMat);
   ...
   componentDidMount() {
      ...
      let rigSize = BounceOld3DView.rigSize;
      let cameraControls;
   ...
   static getDerivedStateFromProps(newProps, oldState) {
      ...
      if (newProps.movie !== oldState.movie) // Complete reset
         rtn = BounceOld3DView.getInitState(newProps.movie);
      return BounceOld3DView.setOffset(rtn, newProps.offset);
   }
      ...
      while (evtIdx + 1 < evts.length && evts[evtIdx + 1].time <= timeStamp) {
         ...
         if (evt.type === BounceMovie.cMakeBarrier
            ...
            let obj = new THREE.Mesh(new THREE.BoxGeometry(width, height,
               6 * ballRadius), BounceOld3DView.flatSteelMat);
         ...
         if (evt.type === BounceMovie.cTargetFade) {
            ...
            ball.position.set(0, BounceOld3DView.rigSize, ballRadius);
